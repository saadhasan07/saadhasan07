...
def create_cv_pdf():
    ...
    # Create the rotated image first, before building the PDF
    ...
    try:
        ...
        if os.path.exists(image_path):
            # Load and rotate the image to correct orientation
            pil_img = PILImage.open(image_path)
            rotated_img = pil_img.rotate(90, expand=True)
            
            # Save rotated image to a temporary file
            rotated_image_path = "rotated_avatar_temp.jpg"
            rotated_img.save(rotated_image_path, "JPEG")
        else:
            print(f"Image file not found: {image_path}")
    except Exception as e:
        print(f"Error preparing profile image: {e}")
    
    # Add profile image at the top
    if rotated_image_path and os.path.exists(rotated_image_path):
        try:
            # Create a simple centered image
            profile_img = Image(rotated_image_path, width=1.2*inch, height=1.2*inch)
